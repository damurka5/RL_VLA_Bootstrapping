#!/usr/bin/env bash
# Resume the pick-up phase from a numbered GRPO checkpoint.
#
# The scratch launcher warm-starts WEIGHTS ONLY from a move-to adapter, which
# also resets the approach curriculum and the optimizer moments -- right when
# pick_up begins, wrong when the question is "what did this one change do?".
# A full resume restores global_step, the curriculum cap and the optimizer
# state, so an intervention like episode_offset_std is the only difference
# between this run and the checkpoint it continues.
#
# MAX_TRAIN_STEPS is the phase target, not a delta: the trainer restores
# global_step from the checkpoint and trains the remainder.
set -euo pipefail
...
# 256, not 512: the 512-world SmolVLA inference activations are the dominant
# GPU peak, and at 512 the combined PyTorch+Warp footprint sat on the card
# limit and Warp OOM'd mid-run once the LoRA backward was added.
...
if [[ -n "${WARMSTART_CHECKPOINT:-}" ]]; then
  # Resume wins over warm-start in the trainer, so setting both would silently
  # ignore one of them. Say so instead.
  echo "This launcher resumes a full checkpoint; do not also set WARMSTART_CHECKPOINT." >&2
  exit 2
fi
...
unset RLVLA_SMOLVLA_WARMSTART_CHECKPOINT
...
printf 'mode=pick_up_full_resume\n'
printf 'checkpoint=%s\n' "$CHECKPOINT"
printf 'run_dir=%s\n' "$RUN_DIR"
...
printf 'episode_offset_std=%s\n' \
  "$(grep -E '^[[:space:]]+episode_offset_std:' "$CONFIG" | tail -1 | sed 's/.*episode_offset_std:[[:space:]]*//')"
printf 'watch: post_grasp_action_z_episode_std, success by start stage, physical_grasp_rate\n'
printf 'command:'
printf ' %q' "${train_cmd[@]}"
printf '\n'
if [[ "$DRY_RUN" == "1" ]]; then
