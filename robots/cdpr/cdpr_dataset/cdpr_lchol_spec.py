...
class CDPRLCHOLSpec:
    ...
    def phase_score(self, trajectory: Sequence[Mapping[str, Any]], option_name: str | None = None) -> float:
        ...
        return float(sparse_success + 0.50 * approach - 0.20 * premature)
            ...
            records.append(
                HindsightBCRecord(
                    ...
                    metadata={
                        ...
                        "prefix_start_timestep": int(start),
                        "prefix_end_timestep": int(end - 1),
                    },
                )
            )
        return records
...
def _pick_predicate(info: Mapping[str, Any]) -> bool:
    if not (_boolish(info.get("grasped")) or _boolish(info.get("caught_object_is_target"))):
        return False
