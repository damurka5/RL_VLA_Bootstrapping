...
| four textured visual-mesh slots | pass |
| forty-four native primitive collider slots | pass |
| all ten RoboCasa visual packs | pass |
...
Local MuJoCo 3.2.4 compilation of both the original robot MJCF and all ten
fixed-topology catalog variants passes. The fixed scene dimensions are
`nq=46`, `nv=40`, `nu=6`, `nbody=24`, `ngeom=88`, `nmesh=11`, `ncam=2`,
`nmat=18`, and `ntex=17`.
...
- per-world mesh `geom_dataid` mutation and BVH refit on A40;
- GPU `contact_force` output for bilateral pad contacts;
...
- four object slots and the ten enabled RoboCasa catalog names;
...
- Unused fixed-topology primitive slots are tiny and parked above the
  ...
  changing topology.
- Objects remain free bodies. There is no centered-close fallback, pose
  pinning, or object teleport during rollout. Grasp evidence requires
  persistent bilateral pad contacts, solved normal forces, stable relative
  pose, and physical lift; release is observed after opening the gripper.
- Training contact handling is exclusively MJWarp/CUDA. The CPU comparison
  fixture is an optional offline validator and is never imported by the
  production trainer.
...
3. `physical_grasp_videos/manifest.json`, MP4s, and CSV: real textures,
   bilateral contacts, solved pad forces, slip, physical lift, and release.
4. Optional `parity.json` and `parity.md`: end-effector, object, tendon,
   gripper, contact, sparse-success, and camera discrepancies from identical
   state.
5. `resume_report.json`: two ranks complete reset, rollout, update,
   checkpoint, and resume with simulator metadata.
6. `benchmark.json` and `benchmark.md`: end-to-end results for 8, 16, 32, 64,
   and 128 worlds/rank.
...
- repository test suite: 370 tests, 7 dependency skips, 0 failures;
- current real-object MJWarp migration module: 27 tests, 2 unavailable-runtime
  skips, 0 failures;
